...
export const LoginForm = ({ stateFromInvitation , invitationToken, invitationResponse }) => {
  console.log("stateFromInvitation in LoginForm ", stateFromInvitation);
  const navigate = useNavigate();
  const [state, setState] = React.useState(stateFromInvitation || "login")
  const [invalidCredential, setInvalidCredentail] = useState(false);
  const [internalServerError, setInternalServerError] = useState(false);
  const {user,login,signup} = useAuth();

  const [formData, setFormData] = React.useState({
    name: '',
    email: invitationResponse?.email? invitationResponse.email : '',
    password: ''
  })

  console.log("invitationResponse in LoginForm ", invitationResponse);
  const handleSubmit = async (e) => {
    e.preventDefault()

    console.log("formData email " + formData.email, "password " + formData.password, "name " + formData.name);
    try {
      let token;

      if (state === "login") {
        const response = await loginUser(formData,invitationToken)
        console.log("Success:", response.data)
        token = response.data.data.token;
        console.log("token ", token);
        const logedinUser = login(token);
        console.log("user object ",logedinUser)
        console.log("user role " ,logedinUser?.role[0])
        const role = logedinUser?.role[0];
        if(role ==="ROLE_ADMIN")navigate("/admin")
        else if(role === "ROLE_MEMBER" || role === "ROLE_TEAM_LEAD")navigate("/user");

      } else {
        const response = await signupUser(formData,invitationToken)
      ...
      console.log("Final error catch:", error.response)
      if (state === "login") setInvalidCredentail(true);
      else {
  ...
  React.useEffect(() => {
  if (invitationResponse?.email) {
    setFormData(prev => ({ ...prev, email: invitationResponse.email }));
  }
}, [invitationResponse]);
    ...
    <div className='w-[32rem] bg-white p-6 rounded-lg shadow-lg' >
      <form
        ...
        <div className="flex items-center w-full mt-4 bg-gray-800 border border-gray-700 h-12 rounded-full overflow-hidden pl-6 gap-2 ">
          ...
          <input type="email" name="email" placeholder="Email id" className="w-full bg-transparent text-white placeholder-gray-400 border-none outline-none " value={invitationResponse?.email? invitationResponse.email : formData.email} readOnly={invitationResponse?.email? true : false} onChange={handleChange} required />
        </div>
        ...
        {invalidCredential && (<p className="text-red-400 text-sm mt-2">Incorrect Email or Password</p>)}
        {internalServerError && (<p className="text-red-400 text-sm mt-2">Internal Server Error please Try again!</p>)}
